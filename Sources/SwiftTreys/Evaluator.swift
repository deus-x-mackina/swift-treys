...
public struct Evaluator {
    ...
    */
    public static func evaluate
        <T: RangeReplaceableCollection>
        (cards: T, board: T? = nil) throws -> HandRank
        ...
        case 5:
            return five(cards: allCards)
        case let x where x == 6 || x == 7:
            return sixOrSeven(cards: allCards)
        default:
            throw SwiftTreysError.invalidNumberOfCards(allCards.count)
        }
    }
    ...
    */
    public static func classifyPokerHand
        <T: RangeReplaceableCollection>
        (cards: T, board: T? = nil) throws -> String
        where T.Element == Card {
    ...
    */
    public static func getClassRank(handRank: HandRank) throws -> ClassRank {
        ...
        case let x where x >= 0 && x <= lt.MAX_STRAIGHT_FLUSH:
            return mtr[lt.MAX_STRAIGHT_FLUSH]!
        case let x where x <= lt.MAX_FOUR_OF_A_KIND:
            return mtr[lt.MAX_FOUR_OF_A_KIND]!
        case let x where x <= lt.MAX_FULL_HOUSE:
            return mtr[lt.MAX_FULL_HOUSE]!
        case let x where x <= lt.MAX_FLUSH:
            return mtr[lt.MAX_FLUSH]!
        case let x where x <= lt.MAX_STRAIGHT:
            return mtr[lt.MAX_STRAIGHT]!
        case let x where x <= lt.MAX_THREE_OF_A_KIND:
            return mtr[lt.MAX_THREE_OF_A_KIND]!
        case let x where x <= lt.MAX_TWO_PAIR:
            return mtr[lt.MAX_TWO_PAIR]!
        case let x where x <= lt.MAX_PAIR:
            return mtr[lt.MAX_PAIR]!
        case let x where x <= lt.MAX_HIGH_CARD:
            return mtr[lt.MAX_HIGH_CARD]!
        default:
            throw SwiftTreysError.invalidHandRankInteger(handRank)
        }
    }
    ...
    */
    public static func handSummary
        <T: RangeReplaceableCollection, U: RangeReplaceableCollection>
        (board: T, hands: U) throws
        where T.Element == Card,
        T.Index == Int, U.Element == T,
        U.Index == Int {
        ...
        for i in 0..<stages.count {
            ...
            // otherwise on all other streets
                ...
                let handResult = try! classRankToString(
                    classRank: getClassRank(
                        handRank: evaluate(
                            cards: hands[winners[0]], board: board)
                    ))
                print("\n\(line) HAND OVER \(line)")
                if winners.count == 1 {
                    print("""
