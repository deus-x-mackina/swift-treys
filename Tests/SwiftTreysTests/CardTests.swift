...
final class CardTests: XCTestCase {
    func testUniqueCardIntegers() throws {
        let integers: [Int] = Rank.allCases.flatMap { rank in
            Suit.allCases.map { suit in Card(rank, suit).binaryInteger }
        }
    ...
    func testCardFromValidStringList() throws {
        ...
        validStrings.forEach { XCTAssertNotNil(Card.fromStringList([$0])) }
    }
    ...
    func testCardFromInvalidStringList() throws {
        ...
        invalidStrings.forEach { XCTAssertNil(Card.fromStringList([$0])) }
    }

    func testStringConversions() throws {
        let cards: [Card] = Rank.allCases.flatMap { rank in
            Suit.allCases.map { suit in Card(rank, suit) }
        }
        cards.forEach {
            let rank = $0.rank
            let suit = $0.suit
            let prettySuit = Card.PRETTY_SUITS[
                Card.CHAR_SUIT_TO_INT_SUIT[Character(suit.rawValue)]!
            ]!
    ...
    let validStrings: [String] = Rank.allCases.flatMap { rank in
        Suit.allCases.map { suit in String(rank.rawValue) + String(suit.rawValue) }
    }
}
